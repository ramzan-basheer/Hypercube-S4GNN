...
from data.data_utils.eeg_data_utils import get_knn_graph


class S4GNN(nn.Module):
    def __init__(
        self,
        input_dim,
        num_nodes,
        ...
        **kwargs
        ...
        self.g_conv = g_conv
        self.fn_connectivity = fn_connectivity
        ...
        if g_conv == "graphsage":
            ...
            for _ in range(num_gnn_layers):
                self.gnn_layers.append(GCNConv(hidden_dim, hidden_dim, **kwargs))
        elif g_conv == "gat":
            for _ in range(num_gnn_layers):
                self.gnn_layers.append(
                    GATv2Conv(
    ...
    def forward(self, data, lengths=None):
        ...
        if self.use_prior:
            ...
            z = x.reshape(batch, num_nodes, -1)
            edge_index, edge_weight, adj_mat = get_knn_graph(
                z,
                self.K,
                dist_measures=[self.fn_connectivity],
        ...
        if self.temporal_pool == "mean":
            if lengths is None:
                ...
                replay_blend = torch.mean(replay_blend, dim=1)
            else:
                x = torch.stack(
                    [
        ...
        for i in range(len(self.gnn_layers)):
            x = self.gnn_layers[i](
                x, edge_index=edge_index, edge_weight=edge_weight.reshape(-1, 1)
            )
